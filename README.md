...
- **AI 润色**：编辑页面新增一键 AI 润色能力，支持预设风格和自定义 systemPrompt，帮你快速美化日记内容。
...
- **2026-06-10**：优化首次打开 进入验证页面 -> token过期 -> 登录页面 的情况，直接首次打开验证token，失效则进入登录页面

- **2026-05-11**：新增 AI 润色能力
  - 编辑页面顶部新增 AI 润色按钮，一键开始润色
  - 支持“预设风格”和“自定义风格”两种模式
  - 自定义模式支持手动输入 `systemPrompt`，并提供示例提示词参考
  - 润色结果可在抽屉中预览，并一键应用回编辑区
  - 统一使用 `POST /api/polish` 接口，增强新建/编辑场景兼容性
